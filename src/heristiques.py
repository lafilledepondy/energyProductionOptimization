...
class MaintenanceHeuristicV2_basic(AbstractMaintenanceHeuristic):
    ...
    def computeProductionPlanLP(self, data: Readingfile, scenario: int, y_it: list[list[int]], x_itk: list[list[tuple[int, int]]]) :
        ...
        smax_i = {
            i: (Smax[i][0] if len(Smax[i]) > 0 else float("inf"))
            for i in I2
        }
        ...
        for i in I2:
            for t in T:
                ...
                if t in selected_start[i]:
                    ...
                    rmax_value = Rmax[i][k_idx] if k_idx < len(Rmax[i]) else data.accessCampaign(i, k_idx).maxrefuel()
                    model.addConstr(
                        r_it[i, t] <= rmax_value,
        ...
        if (
            ...
            obj_value = -1
            p1_solution = {(i,t): 0.0 for i in I1 for t in T}
            p2_solution = {(i,t): 0.0 for i in I2 for t in T}
            r_solution = {}
            s_solution = {(i,t): float(X_i[i]) for i in I2 for t in T}
...
class MaintenanceHeuristicV2MultiCampaign(MaintenanceHeuristicV2_basic):
    ...
    def scheduleMaintenance(self, data: Readingfile) -> tuple[list[list[int]], list[list[tuple[int, int]]]] | None:
        ...
        x_itk = [[] for _ in range(I2)]
        x = [[] for _ in range(I2)]
        h2 = MaintenanceHeuristicV2_basic()
